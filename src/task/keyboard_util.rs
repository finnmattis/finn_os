...
impl Keyboard {
    ...
    pub fn get_key_ev(&self, code: u8) -> Result<KeyEvent, ()> {
        match code {
            0x80..=0xFF => {
                // Release codes
                Ok(KeyEvent::new(self.get_key_code(code - 0x80)?, KeyState::Up))
            }
            _ => {
                // Normal codes
                Ok(KeyEvent::new(self.get_key_code(code)?, KeyState::Down))
            }
        }
    }

    fn get_key_code(&self, code: u8) -> Result<KeyCode, ()> {
        ...
        match code {
            ...
            0x81..=0xD8 => Ok(self.get_key_code(code - 0x80)?),
            _ => Err(()),
        }
    }
